...
So, the disk map 12345 has three files: a one-block file with ID 0, a three-block file with ID 1,
...
To calculate the checksum, add up the result of multiplying each of these blocks' position with the file ID number it contains. The leftmost block is in position 0.
...
Day9ParserState :: struct {
    disk: [dynamic]i64
}

d9LineParser : lineParseFunction : proc(line: string, state: rawptr) {
    state := (^Day9ParserState)(state)
    for char, i in line {
        for j := 0; j < int(char - '0'); j += 1 {
            if i % 2 == 0 {
                append(&state.disk, i64(i / 2))
            }
            else {
                append(&state.disk, -1)
            }
...
d9p1 :: proc(inputPath: string) -> (checksum: u64 = 0) {
    state: Day9ParserState
    defer delete(state.disk)
    parseFileToFunction(inputPath, d9LineParser, &state)

    end := len(state.disk) - 1
    for i := 0; i <= end; i += 1 {
        if state.disk[i] == -1 {
            for ; end > i; end -= 1 {
                if state.disk[end] != -1 {
                    state.disk[i] = state.disk[end]
                    state.disk[end] = -1
                    end -= 1
                    break
                }
        ...
        checksum += u64(i) * u64(state.disk[i])
    }
...
d9p2 :: proc(inputPath: string) -> (checksum: u64 = 0) {
    state: Day9ParserState
    defer delete(state.disk)
    parseFileToFunction(inputPath, d9LineParser, &state)

    for i := len(state.disk) - 1; i >= 0; i -= 1 {
        idValue := state.disk[i]
        if idValue == -1 {
            continue
        }
        ...
        for j := 0; j < i; j += 1 {
            ...
            if gapLength >= blocklength {
                for k := 0; k < blocklength; k += 1 {
                    state.disk[j + k] = idValue
                    state.disk[i - k] = -1
                }
                break
            }
