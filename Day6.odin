...
d6LineParser : lineParseFunction : proc(line: string, state: rawptr) {
    ...
    append(&state^.lines, transmute([]u8)line)
}

d6p1 :: proc(inputPath: string) -> (uniqueTiles: i64 = 1) {
    Direction :: enum {
        UP,
        DOWN,
        LEFT,
    ...
    state: Day6ParserState
    defer delete(state.lines)
    defer for line in state.lines {
        delete(line)
    }
...
From the safety of a supply closet, you time travel through the last few months and record the nightly status of the lab's guard post on
...
d6p2 :: proc(inputPath: string) -> (count: i64 = 0) {
    Direction :: enum {
        UP,
        DOWN,
        LEFT,
