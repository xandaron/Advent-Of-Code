...
and while The Historians continue their search of this historically significant facility, an Elf operating a very familiar printer beckons you over.
...
Day5ParserState :: struct {
    rules: [dynamic][2]i64,
    loadingRules: b8,
    total: i64
}

d5p1LineParser : lineParseFunction : proc(line: string, state: rawptr) {
    state := (^Day5ParserState)(state)
    if line == "" {
    ...
    if state^.loadingRules {
        ...
        rule[0] = stringToInt(numberStrings[0])
        rule[1] = stringToInt(numberStrings[1])
        append(&state.rules, rule)
        delete(numberStrings)
        return
    }
    
    numberStrings := strings.split(line, ",")
    numbers := make([]i64, len(numberStrings))
    defer delete(numbers)
    for str, i in numberStrings {
    ...
    state.total += numbers[len(numbers) / 2]
}

d5p1 :: proc(inputPath: string) -> i64 {
    state: Day5ParserState = {
        loadingRules = true
    }
    defer delete(state.rules)
    parseFileToFunction(inputPath, d5p1LineParser, &state)
    return state.total
}
...
d5p2LineParser : lineParseFunction : proc(line: string, state: rawptr) {
    state := (^Day5ParserState)(state)
    if line == "" {
        state^.loadingRules = false
        return
...
d5p2 :: proc(inputPath: string) -> i64 {
    state: Day5ParserState = {
        loadingRules = true
    }
    defer delete(state.rules)
    parseFileToFunction(inputPath, d5p2LineParser, &state)
    return state.total
}
