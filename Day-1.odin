...
d1p1 :: proc(inputFile: string) -> (sumDistance: i64 = 0) {
    left, right := parseFile(inputFile)
    defer delete(left)
    defer delete(right)
...
Calculate a total similarity score by adding up each number in the left list after multiplying it by the number of times that
...
d1p2 :: proc(inputFile: string) -> (similarityScore: i64 = 0) {
    search :: proc(s: [dynamic]i64, value: i64) -> int {
        max := len(s) - 1
        min := 0
        index := max / 2
        for {
...
parseFile :: proc(filepath: string) -> (left, right: [dynamic]i64) {
